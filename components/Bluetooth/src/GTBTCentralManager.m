...
#import "GTBTDataWriteTask.h"
#import "GTBTDiscoverTask.h"
...
#pragma mark- GTBLePeripheralManager

@interface GTBTCentralManager ()
...
@implementation GTBTCentralManager
{
@private
...
-(instancetype)initWitPeripheralAdvertiseID:(nullable NSString *)advertiseID
{
    ...
    if(self)
    {
        ...
        if(!_centralManager)
        {
            ...
            _state = (GTBTCentralState)_centralManager.state;
        }
...
-(NSError *)scanPeripherals
{
    ...
    if(_state <= GTBTCentralStateUnauthorized)
        errorStr = NSLocalizedString(@"Bluetooth service unavailable", nil);
    else if(_state == GTBTCentralStatePoweredOff)
    ...
    _state = GTBTCentralStateScanning;
...
-(void)stopScanPeripherals
{
    ...
    if(_state == GTBTCentralStateScanning)
        _state = GTBTCentralStatePoweredOn;
    else
        _state = (GTBTCentralState)_centralManager.state;
...
{
    WEAK_SELF;
    [self emunateDelegatesWithBlock:^(id<GTBlePeripheralManagerDelegate> delegate, BOOL *stop) {
...
-(void)inter_failAllServiceDiscoverBlocks:(nonnull NSError *)error forPeripheral:(CBPeripheral *)peripheral
{
    ...
    for( GTBTDiscoverTask *task in allTasks)
    {
        if(task.block)
            task.block(error);
    }
...
-(void)inter_invokeServiceDiscoverBlockForPeripheral:(CBPeripheral *)peripheral withError:(NSError *)error {
    ...
    if(allTasks.count > 0)
    {
        GTBTDiscoverTask *task = allTasks.firstObject;
        if(task.block)
            task.block(error);
...
-(void)inter_discoverServiceWtihTask:(nullable GTBTDiscoverTask *)task forPeripheral:(CBPeripheral *)peripheral
{
...
{
    GTBTDiscoverTask *task = [[GTBTDiscoverTask alloc] init];
    task.block = block;
...
-(void)inter_discoverCharacteristicTask:(GTBTDiscoverTask *)task ofService:(nonnull CBService *)service
{
#if ENABLE_GTBT_LOG
...
-(void)inter_failAllCharacteristicsDiscoverBlocksForPeripheral:(nonnull CBPeripheral *)perpheral
{
    ...
    {
        NSArray *tasks = [self.discoverCharateristicTaskkQueueMap objectsForKey:key];
        for(GTBTDiscoverTask *task in tasks)
        {
            if(task.block)
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverCharacteristicsForService:(CBService *)service error:(nullable NSError *)error
{
    ...
    NSString *key = [self keyForServiceID:service.UUID.UUIDString ofPeripheral:service.peripheral];
    NSArray *tasks = [self.discoverCharateristicTaskkQueueMap objectsForKey:key];
    GTBTDiscoverTask *task = tasks.firstObject;
    if(task)
    {
        if(task.block)
            task.block(error);
    ...
    if(tasks.count > 0)
    {
        GTBTDiscoverTask *task1 = [tasks objectAtIndex:0]; //tasks是拷贝出来的，且有一次对object的移除操作，所以这里index是1
...
- (void)peripheral:(CBPeripheral *)peripheral didWriteValueForCharacteristic:(CBCharacteristic *)characteristic error:(nullable NSError *)error
{
    ...
    if(tasks.count > 0 )
    {
        GTBTDataWriteTask *task = tasks.firstObject;
        GTBTDataWriteTask *continueTask = nil;
...
{
    NSString *k = [self keyForDescriptor:descriptor];
    NSArray <GTBTDataWriteTask *> *allTasks = [self.descriptorWriteBlockMapQueue objectsForKey:k];
    if(allTasks.count > 0 )
    {
        GTBTDataWriteTask *taskToContinue = nil;
        GTBTDataWriteTask *task = allTasks.firstObject;
        if(task.pendingData.length == 0 )
        {
...
-(void)writeData:(nonnull NSData *)data forCharacteristic:(nonnull CBCharacteristic *)chara response:(void(^)(NSError *error))response
{
    ...
    {
        if(response)
            response(GTBT_UNKNOW_ERROR);
        
        return;
    }
    
    GTBTDataWriteTask *task = [[GTBTDataWriteTask alloc] init];
...
-(BOOL)inter_writeCharacTask:(GTBTDataWriteTask *)task forCharacteristic:(CBCharacteristic *)characteristic
{
...
-(void)inter_finisthWriteTask:(GTBTDataWriteTask *)task forCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error
{
    task.isWritting = NO;
...
-(void)inter_failAllWriteBlocksForPeripheral:(CBPeripheral *)peripheral error:(NSError *)error
{
    ...
    for(NSString *key in allKeys)
    {
        ...
        for(GTBTDataWriteTask *tsk in tasks)
        {
...
-(void)inter_writeDescriptorTask:(GTBTDataWriteTask *)task forDescriptor:(nonnull CBDescriptor *)descriptor
{
...
-(void)inter_finishWriteOfTask:(GTBTDataWriteTask *)task forDescriptor:(nonnull CBDescriptor *)descriptor withError:(NSError *)error
{
    task.isWritting = NO;
    if(task.responseBlock)
        task.responseBlock(error);
...
-(void)inter_failAllDescriptorWriteBlocksForPeripheral:(CBPeripheral *)peripheral error:(NSError *)error
{
    ...
    for(NSString *k in allKeys)
    {
        ...
        for(GTBTDataWriteTask *task in allTasks)
        {
            if(task.responseBlock)
                task.responseBlock(error);
        }
...
-(NSData *)extracNextPaketDataFromTask:(GTBTDataWriteTask *)task
{
